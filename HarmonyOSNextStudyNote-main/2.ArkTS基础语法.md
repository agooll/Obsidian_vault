...
这里的“自动装箱”可以先按下面这句话理解：

- 基本类型本来不是对象
- 但在某些需要对象能力的场景里，运行时会临时把它包装成对应对象
...
- `let value: Object = 123`
  这里把基本类型`123`赋给`Object`时，可以理解成发生了自动装箱。
- `text.length`
  `text`本身是基本类型字符串，但依然可以访问属性，这也可以从自动装箱的角度去理解。

和它容易混淆的是“显式装箱”：

```ArkTS
let boxed: Number = new Number(123)
```

这不是自动装箱，而是你自己手动创建了包装对象。
...
如果一开始看不懂箭头函数，可以按下面这条逻辑链路理解：

```TypeScript
const add = (a: number, b: number) => a + b
```

- `const add =`：把一个函数交给变量`add`
- `(a: number, b: number)`：这个函数接收两个参数
- `=>`：可以理解成“接收左边这些输入后，执行右边这段逻辑”
- `a + b`：函数真正要做的事，这里就是返回两数之和

所以上面这句代码，本质就是“定义一个叫`add`的函数，传入`a`和`b`，返回`a + b`”。
...
在ArkTS声明式UI里，箭头函数经常作为“临时函数”直接传给事件：
...
- `onClick(...)`需要接收一个“点击后执行的函数”
- 这个函数只在这里用一次，所以直接写成箭头函数
- 点击按钮时，箭头函数才会执行
- 执行内容是`this.count++`

所以箭头函数最常见的使用场景，不是为了炫技，而是为了把“短小的临时函数”直接写在需要它的位置上。

在事件回调里更常用箭头函数，还有一个很重要的原因：它更适合直接“继承外层上下文”去访问变量。
...
- 外层组件里本来就有`this.count`和`this.message`
- `onClick`里面需要继续使用这些外层数据
- 箭头函数作为一个写在当前作用域里的内部函数，阅读时更顺
- 所以在事件、回调、列表遍历、异步处理里，经常优先写成箭头函数

学习时可以先记一个实用结论：当你看到`onClick(() => {})`、`setTimeout(() => {})`、`arr.map(() => {})`这种写法时，先把它看成“这里临时传了一个函数进去，等之后再执行”。
...
如果这段看不懂，可以把它拆成“外层函数创建变量，内层函数记住变量”这条链路：
...
- 在`f()`内部创建变量`count = 0`
- `f()`返回一个箭头函数
- 这个箭头函数虽然被返回出去了，但它还“记得”外层的`count`
- 第一次调用时，`count`从`0`变成`1`
- 第二次调用时，还是同一个`count`，所以从`1`变成`2`

也就是说，闭包的核心不是“语法复杂”，而是：

- 一个函数返回了另一个函数
- 返回出去的这个函数，还能继续访问外层原本的变量

所以闭包可以先记成一句人话：

“函数跑出原来的作用域以后，仍然记得它出生时周围的变量。”
...
如果把普通字段和静态字段放在一起对比，会更容易理解：

```TypeScript
class Person {
  name: string = 'Tom'               // 普通字段，属于对象
  static school: string = 'HMU'      // 静态字段，属于类
}

let p = new Person()

console.log(p.name)         // 通过对象访问普通字段
console.log(Person.school)  // 通过类名访问静态字段
```
...
所以静态字段不是“每 new 一个对象就复制一份”，而是通常只有一份，所有对象共享它。
...
如果子类有自己的构造函数，并且在构造函数里要使用`this`，那么必须先调用`super()`。
...
这里的顺序不能反过来。可以按下面这条链路理解：

- 创建的是子类对象
- 但子类对象里包含父类继承来的那一部分
- 所以必须先调用`super()`，先让父类部分初始化完成
- 然后才能安全使用`this`去访问或初始化子类自己的字段
...
上面的代码会报错，因为`super()`之前不能使用`this`。

如果做选择题，直接记结论：

- 在子类构造函数里：`super()`必须在`this`之前
- 可以记成一句话：先初始化父类，再操作子类对象
...
看到选择题“关于类中的静态成员（static），以下哪项描述是正确的”，先抓住核心：

`static`修饰的成员，属于类，不属于某个具体对象。
...
如果题目选项是“静态成员可以直接通过类名进行访问，无需创建对象”，那么这就是正确项。

这类题常见的三个错误点也可以顺手记住：

- 错误1：认为静态成员要通过对象访问
  这不对，因为静态成员本来就是类级别的成员。
- 错误2：认为每创建一个对象，静态成员都会复制一份
  这不对，静态成员通常只有一份，所有实例共享。
- 错误3：认为静态方法里可以像实例方法那样直接访问实例属性
  这不对，因为静态方法不依附某个具体对象，它不能把自己当成某个实例来处理。
